...
class Splitter {
  ...
  CreateSliderElement() {
    ...
    slider.style.background = "#d1d1d1";
  ...
  RectB() {
    var content = document.getElementById('area-contents');
    var marginX = (this.sliderWidth / content.clientWidth) * 100;
    var marginY = (this.sliderWidth / content.clientHeight) * 100;

    if (this.direction == Splitter.Direction.Horizontal) {
      return new Rect(
        new Vector2(
          this.rect.position.x,
          this.rect.position.y + (this.rect.size.y * this.percent) / 100.0 + marginY
        ),
        new Vector2(
          this.rect.size.x,
  ...
  static MouseMove(e) {
    ...
    var content = document.getElementById('area-contents');
    var top = ((event.pageY - drag.fromY) / content.clientHeight) * 100;
    var left = ((event.pageX - drag.fromX) / content.clientWidth) * 100;
    ...
    if (drag.splitter.direction == Splitter.Direction.Horizontal) {
      ...
      drag.splitter.percent =
        ((top - drag.splitter.rect.position.y) / drag.splitter.rect.size.y) * 100;
      //alert(top);
    } else {
...
var splitter = new Splitter(Splitter.Direction.Horizontal, areaA, areaB, {
  percent: 50,
  'onResizeElementACallbackFunc' : null,
  'onResizeElementBCallbackFunc' : null
});